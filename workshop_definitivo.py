...
# =================================================================
# 1. RESERVOIR COMPARTIMENTALE (Con Adattamento SFA Lento)
# =================================================================
class CompartmentalReservoir:
    def __init__(self, n_neurons=32, input_dim=1, dt=1.0):
        self.N = n_neurons
        self.dt = dt
        self.tau_m    = 30.0
        self.v_rest   = -70.0
        ...
        # SFA (Memoria a lungo termine per distinguere Transizione vs Abitudine)
        self.tau_adapt = 3500.0  
        self.beta      = 2.0    
        self.tau_s     = 10.0
        
        pesi_casuali    = np.random.randn(self.N, self.N) * 2.0
        maschera_sparsa = np.random.rand(self.N, self.N) < 0.30
        self.W_res = pesi_casuali * maschera_sparsa
        np.fill_diagonal(self.W_res, 0.0)
        
        # FIX 1: Pesi di input scalati per assicurare una dinamica eccitatoria che faccia scattare i neuroni
    ...
    def step(self, input_signal):
        self.v_adapt += (-self.v_adapt) / self.tau_adapt * self.dt
        v_thresh_eff = self.v_thresh + self.v_adapt
        
        # L'input agisce come iniezione di corrente
        external_jump = self.W_in @ input_signal
        
        # FIX 2: L'input esterno alimenta la corrente sinaptica integrata nel tempo
        ...
        self.v_adapt[self.spikes] += self.beta
...
class SpikingReadout:
    ...
    def step(self, reservoir_spikes, target_spikes=None, lr=0.1, train=False): # lr ridotto da 0.5 a 0.1
        self.r_trace += (-self.r_trace + reservoir_spikes.astype(float)) / self.tau_trace * self.dt
        
        input_current = self.W_out @ self.r_trace 
        
        # --- FIX MECCANISMO SST (Dis-Inibizione) ---
        # Se c'è il teacher (train=True), le VIP inibiscono le SST (Nessuna inibizione extra).
        # Se non c'è il teacher (train=False), le SST si attivano e deprimono fortemente la corrente.
        sst_inhibition = 0.0 if train else 30.0  # Questo valore "schiaccia" l'attività verso i 10 Hz
        ...
        if train:
            ...
            self.W_out = np.clip(self.W_out, 0.0, None) 
            error_val = error[0]
...
# FIX 5: Aumentato max_learning_phases da 3 a 100 per catturare tutti i pattern
...
def main():
    ...
    teacher = CompartmentalReservoir(n_neurons=32, input_dim=1)
    ...
    logging.info("Avvio Addestramento (Lazy Teacher Gated)...")
    epochs = 150
    for epoch in range(epochs):
        ...
        if (epoch+1) % 15 == 0 and learning_steps > 0:
    ...
    axes[0].set_title("1. Reservoir Activity")
    for t_idx, neurons in enumerate(plot_res):
        if len(neurons) > 0: axes[0].scatter([t_idx] * len(neurons), neurons, color='black', s=2, marker='|')
    ...
    axes[2].set_title("3. Raster Plot: Target (blu) vs Output (verde)")
    ...
    axes[3].set_title("4. Firing Rate Output (Decadimento Naturale)")
    ...
    axes[4].set_title("5. Errore Continuo")
    ...
    plt.tight_layout()
    filename = "snn_funzionante_definitivo_1.png"
