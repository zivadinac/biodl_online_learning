...
def main() -> None:
    ...
    ax_raster.set_yticklabels(
        [f"{k.upper()} ({rates[k]:.0f} Hz)" for k in ["pyr", "pv", "sst", "vip"]]
    )
    ...
    tr = net.trace("pyr")
    ax_pyr.plot(
        tr["times"], tr["Ibasal"], color="#1E8449", label="I_basal (input + PV)"
    )
    ax_pyr.plot(
        tr["times"], tr["Iapical"], color="#7B241C", label="I_apical (teacher - SST)"
    )
    ax_pyr.plot(
        tr["times"], tr["Isoma_mem"], color="black", lw=0.8, label="I_soma (membrane)"
    )
