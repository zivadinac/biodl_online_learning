...
def draw_pyramidal(ax) -> None:
    ...
    ax.plot(
        [APEX[0], SHAFT_TOP[0]], [APEX[1], SHAFT_TOP[1]], color=GREY, lw=2.5, zorder=2
    )
    # apical tuft (fork)
    for dx in (-0.4, -0.15, 0.15, 0.4):
        ax.plot(
            [SHAFT_TOP[0], dx],
            [SHAFT_TOP[1], TUFT_Y + 0.5],
            color=GREY,
            lw=1.6,
            zorder=2,
        )
...
def main() -> None:
    ...
    draw_pyramidal(ax)
    for centre, color, label in (
        (VIP_P, VIP_C, "VIP"),
        (SST_P, SST_C, "SST"),
        (PV_P, PV_C, "PV"),
    ):
        ax.add_patch(
            Circle(centre, R, facecolor=color, edgecolor="white", lw=2, zorder=4)
        )
        ax.text(
            *centre,
            label,
    ...
    ax.annotate(
        "",
        xy=(BASE_L[0] - 0.05, BASE_L[1] - 0.25),
        xytext=(-1.3, -2.55),
        arrowprops=dict(
            arrowstyle="-|>",
            color=DRIVE_C,
    ...
    ax.annotate(
        "Plastic\nweights",
        xy=(BASE_L[0] - 0.18, BASE_L[1] - 0.18),
        xytext=(-2.05, -1.35),
        fontsize=10.5,
        ha="center",
        va="center",
    ...
    _check(g, "pyr", "sst")
    exc(
        ax, (0.6, -0.6), (SST_P[0] - R - 0.05, SST_P[1] - 0.25), rad=-0.35
    )  # PYR -> SST
    _check(g, "pyr", "pv")
    ...
    ax.add_patch(
        Circle((PV_P[0] + R - 0.02, PV_P[1] + 0.18), 0.09, color=PV_C, zorder=6)
    )
    _check(g, "pyr", "pyr")
    ax.annotate(
        "",
    ...
    handles = [
        ...
        Line2D(
            ...
            marker=">",
            markersize=7,
            label="excitatory (PYR)",
        ),
        Line2D(
            [0], [0], color="#555", lw=2.4, marker="o", markersize=8, label="inhibitory"
        ),
    ]
